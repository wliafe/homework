...
def chn_senti_corp(path, *, batch_size=100, step_size=200):
    ...
    # 将文本拆分为词元并统计频率
    token_count=Counter()
    for item in chn_senti_corp.iloc[:,1].values: token_count.update(str(item))
    
    # 建立词表
    vocab=Vocab(token_count,min_freq=10,reserved_tokens=['<pad>'])
    ...
    return vocab,train_iter,val_iter,test_iter #返回词表和迭代器
